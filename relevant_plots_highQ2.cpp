...
int getBinIndex_xQ2(double xB, double Q2){
    ...
    double bin_xB[][2] = {{5e-5, 0.1}, {0.1, 0.2}, {0.2, 0.4}, {0.4, 1.0}}; // if 18x275
    // bin Q2
...
//------------- Main function
void relevant_plots_highQ2(int target_pdg = 321, const char* inputDir = "/volatile/clas12/lpolizzi/sidis/eic/NC_25.10/18x275/minQ2=1000") {
    ...
    */
    ...
    if (fs::exists(inPath) && fs::is_regular_file(inPath)
        && endsWith(inPath.filename().string(), ".root")) {
        ...
        for (const auto &entry : fs::directory_iterator(inputDirStr)) {
            ...
            if (endsWith(entry.path().filename().string(), ".root")) {
    ...
    if (fileCount == 0) {
        cerr << "No *22.root file found in " << inputDirStr << endl;
        return;
    }

    //--- output file
    TString outputFile = Form("minQ2=1000_epic_%s.root", tag.Data());
    ...
    */
    ...
    double chi_min = 0.1;
    double chi_max = 2000;
    const double xmin_xB = 1e-2, xmax_xB = 1;
    const double xmin_Q2 = 1e3, xmax_Q2 = 1e4;
    ...
    double bin_xB[][2] = {{1e-2, 0.1}, {0.1, 0.2}, {0.2, 0.4}, {0.4, 1}}; // 18x275
    // bin_Q2
    double bin_Q2_full[][2] = {{1, 2}, {2, 5}, {5, 20}, {20, 1000}, {1000, 1e4}};
    //double bin_xB_plot[] = {1e-4, 1e-3, 3e-3, 1e-2, 4e-2, 1}; // 10x100
    //double bin_xB_plot[] = {5e-5, 3e-4, 1e-3, 3e-3, 1e-2, 1}; // 18x275
     double bin_xB_plot[] = {1e-3, 0.1, 0.2, 0.4, 1};
    ...
    for (Long64_t i = 0; i < nEntries_had; i++) {
        ...
        if (i % 100000 == 0) cout << "RECO entry: " << i << "/" << nEntries_had << endl;
            ...
            if(hadron_y <= 0.99 && hadron_y >= 0.01 && good_PID == 0 && rec_pdg == target_pdg && rec_pdg_mc == target_pdg && hadron_z < 1 && hadron_Q2 >= 1){ //same request as in epic_studies + goodPID + specific pdg
                ...
                if(hadron_z <= 0.4){
                    if(hadron_PhT <= 2.5)had_PhTvsZ.Fill(hadron_z, hadron_PhT);
                }
                else had_PhTvsZ.Fill(hadron_z, hadron_PhT);
    ...
    TImage *logo2 = TImage::Open("EPIC-logo_black_small.png");
    ...
    Text_ePIC_eff.DrawLatexNDC(.15,.88,"ePIC");
    ...
    else Text_com_eff.DrawLatexNDC(.15,.85,"e+p, #sqrt{s} = 141 GeV");

    TLatex Text_kinem_eff;
    Text_kinem_eff.SetTextAlign(13);
    Text_kinem_eff.SetTextSize(0.035);
    ...
    Text_ePIC_pur.DrawLatexNDC(.15,.88,"ePIC");
    ...
    Text_kinem_pur.SetTextSize(0.035);
    ...
    set<string> id_box5 = { "_Q2VsXb1"};
    set<string> id_box6 = { "_PhTvsZ1"};
    ...
    for (size_t i = 0; i < hists_had.size(); ++i) {
        ...
        Text_ePIC.DrawLatexNDC(.15,.88,"ePIC");
        ...
        if (inputDir && std::strstr(inputDir, "25.10_10x100") != nullptr) Text_com.DrawLatexNDC(.15,.84,"e+p, #sqrt{s} = 63.2 GeV");
        if (inputDir && std::strstr(inputDir, "25.10_18x275") != nullptr) Text_com.DrawLatexNDC(.15,.84,"e+p, #sqrt{s} = 141 GeV");
        else Text_com.DrawLatexNDC(.15,.84,"e+p, #sqrt{s} = 141 GeV");
        TLatex Text_kinem;
        Text_kinem.SetTextAlign(13);
        Text_kinem.SetTextSize(0.035);
        //Text_kinem.DrawLatexNDC(.15,.78,"DIS kinematic: electron method reconstruction");

        TPad *pad2 = new TPad(Form("pad2_%s", histName.c_str()),Form("pad2_%s", histName.c_str()), 0.14, 0.84, 0.22, 0.94);
        pad2->Draw();
        pad2->cd();
        logo->Draw();
    ...
    // -- bin grid (Q2 vs xB)
    TCanvas *c_bin_xQ2 = new TCanvas(Form("%s_Q2_vs_xB_Bin",tag.Data()), Form("%s Q^{2} vs x_{B} bin",tag.Data()), 800, 600);
    c_bin_xQ2->SetLogz(); 
    had_Q2VsXb.SetStats(0);
    had_Q2VsXb.Draw("colz");
    c_bin_xQ2->SetLogx(); c_bin_xQ2->SetLogy();
    ...
    //Text_ePIC.DrawLatexNDC(.15,.88,"ePIC Performance");  // performance plot
    //Text_ePIC.DrawLatexNDC(.15,.88,"ePIC Internal");  // for internal use only
    Text_ePIC.DrawLatexNDC(.15,.88,"ePIC"); // preliminary released version
    //Text_ePIC.DrawLatexNDC(.15,.88,"ePIC Work in Progress"); // work in progress to be shown outside
    //Text_ePIC.DrawLatexNDC(.15,.88,"ePIC"); // final published version
    TLatex Text_com;
    Text_com.SetTextAlign(13);
    if (inputDir && std::strstr(inputDir, "25.10_10x100") != nullptr) Text_com.DrawLatexNDC(.15,.84,"e+p, #sqrt{s} = 63.2 GeV");
    if (inputDir && std::strstr(inputDir, "25.10_18x275") != nullptr) Text_com.DrawLatexNDC(.15,.84,"e+p, #sqrt{s} = 141 GeV");
    else Text_com.DrawLatexNDC(.15,.86,"e+p, #sqrt{s} = 141 GeV");
    //Text_com.DrawLatexNDC(.15,.8,"L_{proj} = 10 fb^{-1}");
    TLatex Text_kinem;
    Text_kinem.SetTextAlign(13);
    Text_kinem.SetTextSize(0.035);
    ...
    /*
    TPad *padx = new TPad("pad2", "Pad 2", 0.14, 0.84, 0.22, 0.94); // Create a new pad and then draw the image in it
    padx->Draw();
    padx->cd(); // Enter the new pad
    logo->Draw();
    */
    c_bin_xQ2->Update();
    c_bin_xQ2->Write();
    ...
    // -- bin grid (PhT vs z)
    TCanvas *c_bin_zPt = new TCanvas(Form("%s Pt_vs_z_Bin",tag.Data()), Form("%s P_{hT} vs z bin",tag.Data()), 800, 600);
    c_bin_zPt->SetLogz();
    ...
    /*
    TPad *pad3 = new TPad("pad3", "Pad 3", 0.14, 0.84, 0.22, 0.94); // Create a new pad and then draw the image in it
    pad3->Draw();
    pad3->cd(); // Enter the new pad
    logo2->Draw();
    */
    ...
    const int nRows = 1;
    const int nCols = 4;
    //int layout[nRows][nCols] = {{ 0, 1, 2, 3}};


    //--- Efficiency plot for all bins
    TCanvas *c_layout = new TCanvas(Form("%s c_4D_efficiency", tag.Data()), Form("%s All efficiency bins", tag.Data()), 2400, 1200);
    c_layout->cd();
    ...
    double x_min = 1e-2, x_max = 1;
    double y_min = 1e3, y_max = 1e4;
    ...
    // x-axis
    std::vector<double> x_ticks = {0.05, 0.1, 0.2, 0.4, 1.0};

    for (double val : x_ticks) {

        double logv = log10(val);
        double norm;

        // stessa mappa non lineare che usi ora
        if (logv <= -3.0) {
            norm = 2.0 * (logv + 1.0);
        }
        else if (logv <= -2.0) {
        ...
        // tick
        TLine *tick = new TLine(pos, y_ndc_min, pos, y_ndc_min - 0.012);
        tick->SetNDC(true);
        tick->Draw();

        // label (numerica, non 10^x)
        TString label;
        if (val < 0.1)
            label = Form("%.2f", val);
        else if (val < 1.0)
        ...
        TLatex *lab = new TLatex(pos, y_ndc_min - 0.035, label);
    ...
    for (int i : y_ticks) {
        ...
        if (val <= 10.0)
            ...
            norm = 2.0 + (log10(val) - 1.0);
        else
            norm = 3.0 + (log10(val) - 2.0);

        norm /= 4.0;

        double pos = y_ndc_min + norm * (y_ndc_max - y_ndc_min);
        if (val == 1e3) pos = 0.295;
        else if (val == 1e4) pos = 0.75;

        // tick
        TLine *tick = new TLine(x_ndc_min, pos, x_ndc_min - 0.012, pos);
        tick->SetNDC(true);
        tick->Draw();

        // label
        TLatex *lab = new TLatex(x_ndc_min - 0.018, pos, Form("10^{%d}", i));
        lab->SetTextAlign(32);
        lab->SetTextSize(0.02);
        lab->SetNDC(true);
        lab->Draw();
    ...
    // y-axis title
    TLatex *ylabel = new TLatex(x_ndc_min - 0.025, 0.86, "Q^{2} (GeV^{2})");
    ...
    // pad info
    double xMargin = 0.065, yMargin = 0.05;
    ...
    for (int iRow = 0; iRow < nRows; ++iRow) {
        for (int iCol = 0; iCol < nCols; ++iCol) {
            int idx = iCol + 1; // idx is the bin index
